...
public class LoadImageAsync extends AsyncTask<Void, Void, BitmapReturnValue> {
	...
	protected BitmapReturnValue doInBackground(Void... voids) {
		...
		try {
			...
			if (mimeType.equals("image/catrobat-image") || mimeType.equals("application/octet-stream")) {
				returnValue = new BitmapReturnValue(FileIO.getBitmapListFromUri(uri, resolver), null);
			} else {
				if (scaleImage) {
